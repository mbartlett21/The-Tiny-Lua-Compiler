...
-- string.find = lpats.find
-- string.match = lpats.match
-- string.gsub = lpats.gsub
-- string.gmatch = lpats.gmatch
...
local printtheresults = function() end

---[[
do
  local ostring_gsub = string.gsub
  local ostring_find = string.find
  local ostring_match = string.match
  local ostring_gmatch = string.gmatch

  local string_gsub = lpats.gsub
  local string_find = lpats.find
  local string_match = lpats.match
  local string_gmatch = lpats.gmatch
  ...
  string.find = function(s, pat, start, n)
    local s1 = os.clock()
    local r1 = { ostring_find(s, pat, start, n) }
    local s2 = os.clock()
    local r2 = { string_find(s, pat, start, n) }
    local s3 = os.clock()
    ...
    if (s3 - s2) / (s2 - s1) > 1000 and (s3 - s2) > 0.01 then
      local out = '---\n'
      out = out .. 'new ' .. (s3 - s2) .. '\n'
      out = out .. 'old ' .. (s2 - s1) .. '\n'
      if s:find('^a+$') then
      ...
      out = out .. (string.format('%q, ', pat) .. tostring(start) .. ', ' .. tostring(n)) .. '\n'
      print(out)
    end
  ...
  string.match = function(s, pat, start, n)
    local s1 = os.clock()
    local r1 = { ostring_match(s, pat, start, n) }
    local s2 = os.clock()
    local r2 = { string_match(s, pat, start, n) }
    local s3 = os.clock()
    ...
    for i = 1, math.max(#r1, #r2) do
      if r1[i] ~= r2[i] then
        print(string.format('string.match(%q, %q, %s)', s, pat, tostring(start)))
        error('difference in results[' .. i .. ']:\n' .. r1[i] .. '\n' .. r2[i])
      end
    end
...
-- mbartlett tests of gsub
do
  local t = { test = 1, testing = 2, [1] = '[1]', [2] = '[2]', [3] = '[3]', [4] = '[4]' }
  assert(string.gsub('test', '()', t) == '[1]t[2]e[3]s[4]t')
  assert(string.gsub('test', '()', t) == '[1]t[2]e[3]s[4]t')
  assert(string.gsub('testing', '()', t) == '[1]t[2]e[3]s[4]ting')
  assert(string.gsub('testing', '().', t) == '[1][2][3][4]ing')
  assert(string.gsub('testing', '.', t) == 'testing')
  assert(string.gsub('testing', '....', t) == '1ing')
end
...
do   -- new (5.3.3) semantics for empty matches
  ...
  for p, e in string.gmatch(sub, "()%s*()") do
    ...
    -- print(string.format('%q',sub):gsub('\n', 'n'), p, e, string.format('%q', string.sub(sub, i, p - 1)), res)
    i = e
  end
  -- print(res)
  assert(res == "-a-b-c-d-")
...
-- print(t[1])print(t[2])print(t[3])print(t[4])
